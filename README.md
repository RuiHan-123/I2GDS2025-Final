...
* Note that the output and error logs for each cell should be merged into a single combined file, because running 5,000 cells would otherwise generate 5,000 .out files and 5,000 .err files. This can overwhelm the directory and cause severe lag or even force the file browser to crash when opening the folder.
